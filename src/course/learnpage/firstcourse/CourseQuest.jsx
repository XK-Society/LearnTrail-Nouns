...
const questions = [
  {
    question: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Phasellus finibus ex justo, at viverra eros pellentesque in",
    options: ["A", "B", "C", "D"],
    answer: ["A"],
  },
  {
      ...
      <Navbar />
        ...
        <div>
          ...
          <div className="flex flex-col items-center justify-center bg-[#131842] p-4">
            {showScore ? (
              <div className="text-2xl text-white"> You scored {score} out of {questions.length} </div>
            ):(
                ...
                <div className="flex flex-col">
                  {questions[currentQuestion].options.map((option, index) => (
                    ...
                    className={`py-2 px-4 mb-2 rounded ${
                      selectedOption=== option ? 'bg-[#E68369] text-white' : 'bg-gray-200'
                    }`}> {option} 
                    </button>
                  ))}
                </div>
                <button onClick={handleNextQuestion} className="mt-4 py-2 px-4 bg-[#E68369] text-white rounded">
                  Next Question
                </button>
              </div>
            )}
